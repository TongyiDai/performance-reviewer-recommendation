...
> 当前宿主还没有安装 Lark CLI。这个 Skill 需要通过 Lark CLI 读取飞书会议、消息、任务、文档和 OKR 证据；请先安装或启用 Lark CLI，完成后再运行 `lark-cli auth status --json --verify` 验证用户身份。
...
2. `lark-cli auth status --json --verify` reports the intended tenant, `identity: user`, and `verified: true`;
